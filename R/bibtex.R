...
make.bib.entry <- function(x) {
  ...
  if ("date" %in% fields && !"year" %in% fields) {
    # If there is a month entry, complete to a full date for as.Date() (#56)
    if (grepl("^\\d{4}-\\d{2}$", y$date)) y$date <- paste0(y$date, "-01")
    y$year <- tryCatch(format(as.Date(y$date), "%Y"), error = err.fun)
    if (is.null(y[["year"]])) return()
  }

  # Adapt to newer BIBs that are designed for BIBLaTeX (#57)
